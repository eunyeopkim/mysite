...
public class BoardService {
	...
	public Boolean modifyContents(BoardVo boardVo) {
		return boardRepository.modify(boardVo) == 1;
	...
	public Boolean orderNoUpdate(BoardVo boardVo) {
		int count = boardRepository.orderNoUpdate(boardVo.getgNo(), boardVo.getoNo());
		return count == 1; 
	}

	public Map<String, Object> getContentsList(int  currentPage, String keyword) {
		//1. 페이징을 위한 기본 데이터 계산
				int totalCount = boardRepository.getCount( keyword ); 
