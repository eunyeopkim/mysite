...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(
		...
		// 4. Method의 @Auth가 없으면 Type에 붙어 있는지 확인한다 (과제)
		if(auth == null) {
			auth = handlerMethod.getMethod().getDeclaringClass().getAnnotation(Auth.class);
		}
		
		// 5. Type이나  Method에 둘다 @Auth가 적용이 안되어 있는 경우,
		if(auth == null) {
			return true;
		}
		...
		// 6. 권한(Authorization) check를 위해서 @Auth의 role 가져오기("USER","ADMIN")
		...
		if("USER".equals(role)) {
			...
			// 반드시 authUser의 role이 "ADMIN" 여야 한다.
